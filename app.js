...
function setMsg(el, text, isError = false) {
  el.textContent = text || '';
  el.style.color = isError ? '#fecaca' : 'rgba(255,255,255,0.65)';
}
...
async function loadProgramacao() {
  ...
  const list = Array.isArray(treinos) ? treinos : [];
  for (const s of progSelects) {
    fillSelectOptions(s.el, list);
  }
  ...
  for (const r of progRows || []) {
    byDay.set(Number(r.dia_semana), r.treino_id);
  }

  for (const s of progSelects) {
    if (!s.el) continue;
    s.el.value = byDay.get(s.dia) || '';
  }
...
if (programacaoForm) {
  programacaoForm.addEventListener('submit', async (e) => {
    ...
    for (const s of progSelects) {
      if (!s.el) continue;
      const treinoId = s.el.value;

      if (!treinoId) {
        // limpar programação do dia
        const { error } = await supabase.from('treinos_programacao').delete().eq('dia_semana', s.dia);
        if (error) {
          setMsg(programacaoMsg, error.message, true);
          return;
        }
        continue;
      }
